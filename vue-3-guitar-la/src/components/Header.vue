...
defineEmits(["eliminar-carrito", "decrementar-carrito", "aumentar-carrito", "vaciar-carrito"]);

const totalPagar = computed(() => {
    return prop.propCarrito.reduce((total, producto) => total + (producto.cantidad * producto.precio), 0)
})
                                ...
                                <tbody>
                                    ...
                                    >
                                        ...
                                        <td class="flex align-items-start gap-4">
                                            <button type="button" class="btn btn-dark" @click="$emit('decrementar-carrito', product.id)">
                                                -
                                            </button>
                                            {{ product.cantidad }}
                                            <button type="button" class="btn btn-dark" @click="$emit('aumentar-carrito', product.id)">
                                                +
                                            </button>
                                        </td>
                                        <td>
                                            <button class="btn btn-danger" type="button" @click="$emit('eliminar-carrito', product.id)">
                                                X
                                            </button>
                                        </td>
                                    </tr>
